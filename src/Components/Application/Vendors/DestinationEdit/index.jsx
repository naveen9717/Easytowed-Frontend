...
const DestinationEdit = () => {
  ...
  const getVenueFromApi = async() => {
    await axios.get(`${process.env.REACT_APP_NODE_BACKEND_URL}/api/destination/profile/${userId}`)
 ...
 useEffect(() => {
  ...
  const AddProject = async(data) => {
   ...
   if(selectedImages){
    for (let i = 0; i < selectedImages.length; i++){
      ...
      formData.append("folder", `Easytowed/Destination/`);
  ...
  console.log('selectedImages,files',selectedImages.length+','+files.length)
    if (files.length === selectedImages.length) {
      ...
      // console.log('updatedata',data)
          axios.put(`${process.env.REACT_APP_NODE_BACKEND_URL}/api/destination/update/${userId}`, data).then((response) => {
  ...
  return (
    <Fragment>
      <Breadcrumbs parent='Destination' title='Destination' mainTitle='Destination Edit' />
