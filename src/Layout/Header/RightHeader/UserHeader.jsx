  ...
  return (
    <li className="profile-nav onhover-dropdown pe-0 py-0">
      <div className="media profile-media">
        <Image
          attrImage={{
            className: "b-r-10 m-0 w-25",
            src: `${currentUser ? currentUser.profilePic : profile}`,
            alt: "",
          }}
        />
        <div className="media-body">
          <span>{currentUser ? currentUser.name : name}</span>
      ...
      <UL attrUL={{ className: "simple-list profile-dropdown onhover-show-div" }}>
        <LI
          attrLI={{
            onClick: () => UserMenuRedirect(`${process.env.PUBLIC_URL}/users/edit/`),
          }}>
          <User />
          <span>{Account} </span>
        </LI>
